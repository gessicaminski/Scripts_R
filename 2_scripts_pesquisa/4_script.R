...
## primeiro transformamos em data frame - mtx125n o doc aqui no caso 
# flexmix

mtx125n$tc_asin <- asin(sqrt(mtx125n$tc/100))

nsample <- round(0.001*nrow(mtx125n)) # sampling by 0.1% of the total number of points
if (nrow(mtx125n) > nsample) {  
  set.seed(90) ## esse aqui você coloca  pa fixar um dado 
  mtx125ns <- mtx125n[sample(nrow(mtx125n), nsample),];
}

# effect of arcsin transformation
plot(mtx125ns$tc/100, mtx125ns$tc_asin)
   #main = "Histogram of Savanna and Florest (ZTE 67 - MT)",
  # xlab = "Tree Cover (arcsine transformation)")
#+   ylab = "Waiting time to next eruption (min)")

mSA = stepFlexmix(tc_asin ~ 1, data = mtx125ns, k = 1:5, nrep = 5);
# list BIC criterions # aqui voce escolhe qual quer usar nomralmente usa o BIC, eu testei os tres e foram parecidos
plot(mSA)
BIC(mSA)
AIC(mSA)
ICL(mSA)
...
hist(mtx125ns$tc_asin, 15, freq=FALSE, col="gray", xaxt="n", 
     ...
     main = "Histogram of Savanna and Florest (ZTE 185 - MT)",
     xlab = "Tree Cover (arcsine transformation)")
...
hist(mtx125ns$tc_asin, 15, freq=FALSE, col="gray", xaxt="n", 
     xlim=c(0, asin(sqrt(1))), 
     main = "Histogram of Savanna and Florest (ZTE 188 - MT)",
     xlab = "Tree Cover (arcsine transformation)")
axis(1, at = asin(sqrt(xx1)))
...
y1 = table(clusters(m1))[1]/nrow(mtx125ns)*dnorm(x1, mean = parameters(m1)[1,1],
                                                 sd = parameters(m1)[2,1]);
